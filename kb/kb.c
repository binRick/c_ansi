...
enum kb_csv_cols_t kb_csv_col_name_id(const char *name){
  for(int i=0;i<KB_CSV_COLS_QTY;i++)
    if(stringfn_equal(name,csv_cols[i]))
      return(i);
...
char *kb_get_config_csv(){ return gkb_locations_csvData; }
char *kb_get_config_json(){ return gkb_locations_jsonData; }
char *kb_get_config_yaml(){ return gkb_locations_yamlData; }
